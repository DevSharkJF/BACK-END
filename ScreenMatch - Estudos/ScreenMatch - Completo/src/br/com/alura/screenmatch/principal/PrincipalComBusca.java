...
/*  Importa a biblioteca Gson, que converte objetos Java em JSON e vice-versa
    Converte a resposta JSON da API em um objeto Java do tipo Titulo
*/
...
/* import java.io.IOException;: Importa a classe IOException, que é uma exceção verificada em Java
    Ela é lançada para indicar que ocorreu um erro de entrada/saída (I/O), como falhas ao acessar arquivos ou recursos de rede
    Nesse código, é usada porque o metodo HttpClient.send pode lançar essa exceção

    import java.net.URI;: Importa a classe URI, que representa um Identificador Uniforme de Recursos (URI)
    Nesse código, é usada para criar o URI da API que será acessada
*/
...
public class PrincipalComBusca {
    public static void main(String[] args) throws IOException, InterruptedException {
        HttpClient client = HttpClient.newHttpClient();
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create("https://www.omdbapi.com/?t=nome_filme&apikey=numero_api")).build();
        HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString());
